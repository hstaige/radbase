...
iso_df['idx'] = iso_df.index
rel_df = pd.read_csv('relative_radii.csv')


def compare_rel_results(filename, rel1, rel2, iso, lsuffix='x', rsuffix='y'):
    rel1, rel2 = rel1.copy(), rel2.copy()
    rels = [rel1, rel2]
    for i, rel in enumerate(rels):
        if ('Z1' not in rel.columns or 'A1' not in rel.columns) and 'iso_idx1' not in rel.columns:
            raise ValueError(f'one of the dfs is missing \'A1\' or \'Z1\' or \'iso_idx1\'')
        if ('Z2' not in rel.columns or 'A2' not in rel.columns) and 'iso_idx2' not in rel.columns:
            raise ValueError(f'one of the dfs is missing \'A2\' or \'Z2\' or \'iso_idx2\'')

        if 'Z1' not in rel.columns or 'A1' not in rel.columns:
            merge_df = pd.merge(rel, iso, left_on='iso_idx1', right_on='idx')
            rel = merge_df.drop(columns=['iso_idx1', 'idx'])
            rel = rel.rename(columns={'Z': 'Z1', 'A': 'A1'})


        if 'Z2' not in rel.columns or 'A2' not in rel.columns:
            merge_df = pd.merge(rel, iso, left_on='iso_idx2', right_on='idx')
            rel = merge_df.drop(columns=['iso_idx2', 'idx'])
            rel = rel.rename(columns={'Z': 'Z2', 'A': 'A2'})

        rel = rel.astype({'Z1': int, 'A1': int, 'Z2': int, 'A2': int})
        rels[i] = rel

    merge_df = pd.merge(*rels, on=['Z1', 'A1', 'Z2', 'A2'], suffixes=('_' + lsuffix, '_' + rsuffix))
    merge_df['diff_dR'] = merge_df[f'dR_{lsuffix}'] - merge_df[f'dR_{rsuffix}']
    merge_df['diff_DdR'] = merge_df[f'DdR_{lsuffix}'] - merge_df[f'DdR_{rsuffix}']

    columns = ['Z1', 'A1', 'Z2', 'A2', f'dR_{lsuffix}', f'DdR_{lsuffix}', f'dR_{rsuffix}', f'DdR_{rsuffix}',
               'diff_dR', 'diff_DdR']
...
def compare_abs_results(filename, absl1, absl2, iso, lsuffix='x', rsuffix='y'):
    ...
    merge_df['diff_R'] = merge_df[f'R_{lsuffix}'] - merge_df[f'R_{rsuffix}']
    merge_df['diff_DR'] = merge_df[f'DR_{lsuffix}'] - merge_df[f'DR_{rsuffix}']

    columns = ['Z', 'A', f'R_{lsuffix}', f'DR_{lsuffix}', f'R_{rsuffix}', f'DR_{rsuffix}',
               'diff_R', 'diff_DR']
    merge_df = merge_df[columns]
    ...
    with pd.ExcelWriter(filename, engine='openpyxl', if_sheet_exists="replace", mode=mode) as writer:
        print(writer.sheets)
...
rel_noniso_df.drop(rel_noniso_df.tail(2).index, inplace=True)
rel_noniso_df.dropna(how='any', subset=['dR+'], inplace=True)
rel_noniso_df.ffill(inplace=True)
...
absl_noniso_df.rename(columns={'R+': 'R', 'DR+': 'DR'}, inplace=True)
